...
export default class ProductForm {
  ...
  async save() {
    ...
    return fetchJson(`${ BACKEND_URL }/api/rest/products`, {
      ...
      .catch(error => {
        console.error('Ошибка сохранения данных продукта');
      })
      .finally(() => {
        this.hideLoading();
      });
  ...
  updateImages(imagesList) {
    if (imagesList && imagesList.length) {
      this.fields.imageListContainer.innerHTML = `<ul class='sortable-list'>
        ${ imagesList.map(this.getImageTemplate).join('') }
        </ul>`;
    }
  }
